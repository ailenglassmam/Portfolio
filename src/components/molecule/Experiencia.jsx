...
export default function Experiencia() {
  ...
  const trabajos = trabajosData.map(trabajo => {
    return (
      <div className='min-w-[300px] md:min-w-[370px] border-2 border-black rounded-[30px] transition md:hover:shadow-[7px_7px_0px_rgba(0,0,0,0.25)] duration-150' key={trabajo.id}>
        ...
        <div className='w-full h-[70%] p-7'>
          ...
          <span className={`text-xs px-3 py-1 border-2 border-black rounded-md ${trabajo.tipo === 'Diseño Industrial' ? 'bg-[#99B7DD]' : trabajo.tipo === 'Diseño Gráfico' ? 'bg-[#F8D44C]' : trabajo.tipo === 'Ilustración' ? 'bg-[#EA7A54]' : 'bg-[#A9D86C]'}`}>{trabajo.tipo}</span>
          <div className='pt-7 text-right'>
          <a href={trabajo.link}>Ver Proyecto</a>
        </div>
        </div>
  ...
  return (
    <section id='Experiencia' className='mx-auto py-32 h-fit md:h-screen'>
