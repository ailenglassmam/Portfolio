...
export default function Formacion() {
      ...
      formacion.tipologia === "Titulacion" ? (
        <div className="w-full p-7 border-2 xl:border-[3px] border-black rounded-[30px] transition md:hover:shadow-[7px_7px_0px_rgba(0,0,0,0.25)] duration-150" key={formacion.id}>
          ...
          <div className="mt-5 text-right">
            <span className={formacion.estado === "Finalizado" ? "text-xs px-3 py-1 border-2 xl:border-[3px] border-black rounded-md bg-[#99B7DD]" : "text-xs px-3 py-1 border-2 xl:border-[3px] border-black rounded-md bg-[#FEDF3E]"}>{formacion.estado}</span>
          </div>
        </div>
      ) : null
    )
  ...
  return (
    <section id="Formación" name="section2" className="mx-auto pt-32 h-fit flex flex-col justify-center items-center px-6 sm:px-16 xl:px-24">
