  ...
  const cursos = formacionData.map(formacion => {
    return (
      ...
      <div className='mt-[18px] text-right'>
        <a href={formacion.link} className='px-10 py-2 border-2 border-black rounded-md transition md:hover:shadow-[7px_7px_0px_rgba(0,0,0,0.25)] duration-150' target='_blank'>Ver Certificación</a>
      </div>
    </div>
    ) : null
    )
